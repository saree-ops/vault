...
export default function ProductDetailModal({ product, onClose, onChanged, onDeleted }) {
  ...
  async function downloadZip() {
    ...
    try {
      const zip = new JSZip()
      const safe = String(product.design_number).replace(/[^a-zA-Z0-9-_ ]/g, '_').trim() || 'design'
      let i = 1
      for (const m of media) {
        const res = await fetch(mediaUrl(m.r2_key))
        if (!res.ok) continue
        const blob = await res.blob()
        const extn = (m.r2_key.split('.').pop() || 'bin').toLowerCase()
        zip.file(m.kind === 'video' ? `${safe}-video.${extn}` : `${safe}-${i++}.${extn}`, blob)
      }
    ...
    } catch (e) { setError('Download failed.') } finally { setZipping(false) }
  }
            ...
            {!editing ? (
              <>
                ...
                <div className="flex items-center gap-3">
                  <button onClick={downloadZip} disabled={zipping} className="text-sm font-medium text-ink/70 transition hover:text-ink disabled:opacity-40">{zipping ? 'Zipping…' : 'Download'}</button>
